...
import { classSet } from 'c/conaB2BUtil';
import { normalizeString, findAllTabbableElements, getElementWithFocus, makeEverythingExceptElementInert, restoreInertness } from 'c/conaB2BUtil';

/**
 * A panel used to display content in a layer over the app.
 * */

export default class ConaB2BDialog extends LightningElement {
    labelClose = 'close';
    /**
    ...
    renderedCallback() {
        ...
        if (this._queueShow) {
            this._savedInertElements = makeEverythingExceptElementInert(this.template.host);
    ...
    _focusFirstElement() {
        const dialogContentElement = this.template.querySelector('[data-content-container]');

        const allTabbableElements = findAllTabbableElements(dialogContentElement);
        ...
        const autoFocus = this.dispatchEvent(new CustomEvent('autofocus', { cancelable: true }));
    ...
    _returnFocus() {
        const returnFocus = this.dispatchEvent(new CustomEvent('returnfocus', { cancelable: true }));
        ...
        if (this._savedActiveElement && document.body.contains(this._savedActiveElement)) {
    ...
    _dispatchCancelEventAndClose() {
        const close = this.dispatchEvent(new CustomEvent('cancel', { cancelable: true }));
        if (close) {
            this.close();
        }
    }
